...
| `docs/prompt/phase35-master-meta-final-release.prompt` | Phase 35 master final release prompt with DeepSeek V4 Flash Free split workflow. |
...
|---|---|---|
| 35.1 | `docs/prompt/phase35.1-backend-release-hardening.prompt` | Backend release hardening, RBAC, safety config, API checks, backend tests. |
| 35.2 | `docs/prompt/phase35.2-ai-trader-final-polish.prompt` | AI Trader simulation-only final polish, API/UI/tests/docs. |
| 35.3 | `docs/prompt/phase35.3-frontend-dashboard-ui-polish.prompt` | Frontend dashboard/UI/UX polish and frontend validation. |
| 35.4 | `docs/prompt/phase35.4-docs-runbooks-api-examples.prompt` | Docs, runbooks, README, API examples, troubleshooting. |
| 35.5 | `docs/prompt/phase35.5-makefile-ci-maintenance-validation.prompt` | Makefile, CI, maintenance, safety-scan, full validation. |
| 35.6 | `docs/prompt/phase35.6-final-report-release-handoff.prompt` | Final report and release handoff. |
...
```bash
cd ~/zdash
codex "$(cat docs/prompt/phase35.1-backend-release-hardening.prompt)"
```

Then continue:

```bash
codex "$(cat docs/prompt/phase35.2-ai-trader-final-polish.prompt)"
codex "$(cat docs/prompt/phase35.3-frontend-dashboard-ui-polish.prompt)"
codex "$(cat docs/prompt/phase35.4-docs-runbooks-api-examples.prompt)"
codex "$(cat docs/prompt/phase35.5-makefile-ci-maintenance-validation.prompt)"
codex "$(cat docs/prompt/phase35.6-final-report-release-handoff.prompt)"
```

If shell prompt length is unstable, pipe the file:

```bash
cat docs/prompt/phase35.1-backend-release-hardening.prompt | codex
```
...
- tracked `docs/prompt/codex-runs/`
