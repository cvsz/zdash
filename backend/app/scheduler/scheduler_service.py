...
class SchedulerService:
    ...
    def start(self) -> dict:
        ...
        if not self.scheduler.running:
            ...
            event_bus.emit('scheduler.started', 'SchedulerService', 'Scheduler started', {'timezone': self.settings.scheduler_timezone})
    ...
    def stop(self) -> dict:
        if self.scheduler.running:
            ...
            event_bus.emit('scheduler.stopped', 'SchedulerService', 'Scheduler stopped', {})
        return self.get_status()
    ...
    def create_job(self, request: CreateJobRequest) -> ScheduledJob:
        ...
        try:
            ...
            raise
        event_bus.emit('scheduler.job.created', 'SchedulerService', 'Scheduler job created', {'job': job.model_dump(mode='json')})
        return job
    ...
    def run_job(self, job_id: str, manual: bool = True) -> JobRunResult:
        ...
        if not job.enabled:
            return self._record_skipped_run(job, 'disabled', 'Job is disabled and cannot be executed.')

        if job.status == JobStatus.paused:
            return self._record_skipped_run(job, 'skipped', 'Job is paused and cannot be executed.')

        self.store.update_job(job.id, {'status': JobStatus.running})
        event_bus.emit('scheduler.job.updated', 'SchedulerService', 'Scheduler job updated', {'job_id': job.id, 'status': JobStatus.running.value})
        event_bus.emit('scheduler.job.started', 'SchedulerService', 'Scheduler job started', {'job_id': job.id, 'manual': manual})
        ...
        event_bus.emit(
            ...
            'SchedulerService',
            'Scheduler job updated',
            {'job_id': job.id, 'status': updated_job.status.value, 'run_count': updated_job.run_count, 'fail_count': updated_job.fail_count},
        )
        ...
        if result.status in {'skipped', 'blocked_by_risk', 'disabled', 'blocked'}:
            event_bus.emit(
                ...
                'SchedulerService',
                'Scheduler job skipped',
                {'job_id': job.id, 'status': result.status, 'message': result.message, 'output': result.output},
            )
        elif result.ok:
            event_bus.emit(
    ...
    def pause_job(self, job_id: str) -> ScheduledJob:
        ...
        event_bus.emit('scheduler.job.updated', 'SchedulerService', 'Scheduler job updated', {'job_id': job_id, 'status': job.status.value})
        event_bus.emit('scheduler.job.paused', 'SchedulerService', 'Scheduler job paused', {'job_id': job_id})
        return job
    ...
    def resume_job(self, job_id: str) -> ScheduledJob:
        ...
        if runtime_job is not None:
            ...
            self._schedule_job_if_needed(job)
        event_bus.emit('scheduler.job.updated', 'SchedulerService', 'Scheduler job updated', {'job_id': job_id, 'status': job.status.value})
        event_bus.emit('scheduler.job.resumed', 'SchedulerService', 'Scheduler job resumed', {'job_id': job_id})
        return job
    ...
    def delete_job(self, job_id: str) -> bool:
        ...
        if deleted:
            event_bus.emit('scheduler.job.deleted', 'SchedulerService', 'Scheduler job deleted', {'job_id': job_id})
        return deleted

    def list_runs(self, job_id: str | None = None) -> list[JobRunResult]:
        return self.store.list_runs(job_id=job_id)

    def _record_skipped_run(self, job: ScheduledJob, status: str, message: str) -> JobRunResult:
        ...
        event_bus.emit('scheduler.job.skipped', 'SchedulerService', 'Scheduler job skipped', {'job_id': job.id, 'status': status, 'message': message})
        return result
    ...
    def _schedule_job_if_needed(self, job: ScheduledJob) -> None:
        should_schedule = job.enabled and job.status != JobStatus.paused and job.schedule_type in {
        ...
        if job.schedule_type == ScheduleType.interval and job.interval_seconds is not None:
            ...
            trigger = CronTrigger.from_crontab(job.cron, timezone=self.settings.scheduler_timezone)
            self.scheduler.add_job(
                self.run_job,
                trigger,
                id=job.id,
