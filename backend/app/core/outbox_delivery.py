...
def plan_retry(
    *,
    ...
    ``attempt_count`` is the number of delivery attempts already recorded,
    including the attempt that just failed. Once it reaches ``max_attempts``
    the message moves to the dead-letter state.
    """
    if attempt_count < 1:
