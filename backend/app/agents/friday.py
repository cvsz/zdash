...
class FridayAgent(BaseAgent):
    ...
    def run_task(self, task: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        try:
            if task == 'health':
                ...
                result = {'jobs': [job.model_dump(mode='json') for job in self.list_jobs()]}
            elif task == 'create_job':
                result = {'job': self.create_job(context).model_dump(mode='json')}
            elif task == 'run_job':
                result = {'result': self.run_job(str(context.get('job_id'))).model_dump(mode='json')}
            elif task == 'pause_job':
                result = {'job': self.pause_job(str(context.get('job_id'))).model_dump(mode='json')}
            elif task == 'resume_job':
                result = {'job': self.resume_job(str(context.get('job_id'))).model_dump(mode='json')}
    ...
    def create_job(self, request: CreateJobRequest | dict[str, Any]):
        req = request if isinstance(request, CreateJobRequest) else CreateJobRequest.model_validate(request)
