...
class SocialService:
    def __init__(
        self, store: InMemoryContentStore, adapter: SocialMediaAdapter | None = None
    ) -> None:
        self.store = store
    ...
    def approve_content(self, request: ApproveContentRequest):
        ...
        event_bus.emit(
            ...
            "SocialService",
            "Content approved",
            {"content_id": item.id, "approved_by": request.approved_by},
        )
        return item
    ...
    def publish_content(self, request: PublishContentRequest):
        ...
        for p in request.platforms or item.platforms:
            res = self.adapter.publish(
                p,
                item.edited_text or item.draft_text or item.topic,
                item.graphic_asset_url,
                item.metadata,
            )
            results.append(res)
        event_bus.emit(
