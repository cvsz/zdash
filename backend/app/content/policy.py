...
class ContentPolicyChecker:
    def check_text(self, text: str, context: dict | None = None) -> dict:
        ...
        patterns = [r'api[_-]?key\s*[:=]', r'access[_-]?token\s*[:=]', r'password\s*[:=]', r'-----BEGIN (RSA|EC|OPENSSH) PRIVATE KEY-----']
        ...
        if re.search(r'guaranteed\s+profit|risk[- ]?free\s+profit|100%\s+win', t, re.IGNORECASE):
            blocked.append('misleading_profit_claim')
            notes.append('Misleading guaranteed-profit language detected.')
        if re.search(r'official\s+zDash\s+support\s+agent|i\s+am\s+your\s+broker', t, re.IGNORECASE):
        ...
        if re.search(r'trad(e|ing)|market|backtest|strategy', t, re.IGNORECASE):
            ...
            if not re.search(r'educational|not financial advice|simulation|past performance', t, re.IGNORECASE):
                warnings.append('missing_risk_disclaimer')
        passed = not blocked
        if not passed:
            notes.extend(blocked)
        return {'passed': passed, 'notes': notes, 'blocked_terms': blocked, 'warnings': warnings}
