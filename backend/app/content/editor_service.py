...
class EditorService:
    def __init__(self, store: InMemoryContentStore, policy: ContentPolicyChecker | None = None) -> None:
        self.store = store
    ...
    def create_draft(self, request: CreateContentRequest):
        ...
        event_bus.emit('content.draft.created', 'EditorService', 'Draft created', {'content_id': item.id})
    ...
    def edit_content(self, request: EditContentRequest):
        ...
        if 'trad' in edited.lower() or 'market' in edited.lower() or 'backtest' in edited.lower():
            edited += ' Educational use only. Past performance does not guarantee future results.'
        patch = {'edited_text': edited, 'status': 'edited', 'tone': request.tone or item.tone, 'language': request.language or item.language}
        item = self.store.update_item(item.id, patch)
        event_bus.emit('content.edited', 'EditorService', 'Content edited', {'content_id': item.id})
    ...
    def generate_variants(self, content_id: str, count: int = 3) -> list[str]:
        ...
        for _ in variants:
            event_bus.emit('content.variant.generated', 'EditorService', 'Variant generated', {'content_id': content_id})
        return variants

    def _check_policy(self, content_id: str, text: str):
        result = self.policy.check_text(text)
        event_bus.emit('content.policy.checked', 'EditorService', 'Policy checked', {'content_id': content_id, **result})
        if not result['passed']:
            event_bus.emit('content.policy.failed', 'EditorService', 'Policy failed', {'content_id': content_id, **result})
        return self.store.update_item(content_id, {'policy_passed': result['passed'], 'policy_notes': result['notes'] + result['warnings']})
