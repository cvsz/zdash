...
class TapoAdapter(IoTAdapter):
    ...
    def _credentials_present(self) -> bool:
        return bool(self.settings.tapo_username and self.settings.tapo_password and self.settings.tapo_device_ip)

    def _simulated(self, device_alias: str, action: str, message: str, output: dict | None = None) -> IoTActionResult:
    ...
    def _blocked(self, device_alias: str, action: str, message: str, output: dict | None = None) -> IoTActionResult:
        return IoTActionResult(
            ok=False,
    ...
    def get_status(self, device_alias: str) -> IoTActionResult:
        ...
        if self.settings.iot_dry_run or not configured:
            reason = 'Dry-run status simulated.' if self.settings.iot_dry_run else 'Credentials missing; safe status mock returned.'
    ...
    def turn_on(self, device_alias: str) -> IoTActionResult:
        if self.settings.iot_dry_run:
            return self._simulated(device_alias, 'turn_on', 'Dry-run turn_on simulated.', {'simulated': True})

        if not self._credentials_present():
            return self._simulated(device_alias, 'turn_on', 'Missing Tapo credentials; safe turn_on simulation returned.', {'simulated': True})
    ...
    def turn_off(self, device_alias: str) -> IoTActionResult:
        if self.settings.iot_dry_run:
            return self._simulated(device_alias, 'turn_off', 'Dry-run turn_off simulated.', {'simulated': True})

        if not self._credentials_present():
            return self._simulated(device_alias, 'turn_off', 'Missing Tapo credentials; safe turn_off simulation returned.', {'simulated': True})
