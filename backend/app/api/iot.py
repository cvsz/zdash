...
def power_cycle(req: IoTPowerCycleRequest | None = None) -> dict:
    ...
    request = req or IoTPowerCycleRequest(device_alias=settings.tapo_device_alias, confirmation=False)
    result = _service().power_cycle(device_alias=request.device_alias, confirmation=request.confirmation)
    return ok({'result': result.model_dump(mode='json')})
