...
class KillSwitch:
    ...
    def should_trigger(self, drawdown: DrawdownResult) -> bool:
        if drawdown.total_drawdown_percent >= self.settings.emergency_kill_switch_drawdown_percent:
            return True
        if self.settings.hard_halt_on_daily_drawdown and drawdown.daily_drawdown_percent >= self.settings.max_daily_drawdown_percent:
            return True
        return False
