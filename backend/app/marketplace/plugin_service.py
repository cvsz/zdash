...
def list_installations(org_id:str): return [i for i in INSTALLS if i['organization_id']==org_id]
def install_plugin(organization_id,plugin_id,workspace_id,config=None):
    item={'id':f'inst-{len(INSTALLS)+1}','organization_id':organization_id,'workspace_id':workspace_id,'plugin_id':plugin_id,'enabled':False,'config':config or {}}
    INSTALLS.append(item); return item
def enable_plugin(installation_id):
    for i in INSTALLS:
        if i['id']==installation_id: i['enabled']=True; return i
    return None
def disable_plugin(installation_id):
    for i in INSTALLS:
        if i['id']==installation_id: i['enabled']=False; return i
    return None
def uninstall_plugin(installation_id):
    global INSTALLS; INSTALLS=[i for i in INSTALLS if i['id']!=installation_id]; return {'ok':True}
