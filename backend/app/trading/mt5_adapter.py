...
class MT5Adapter:
    ...
    def _credentials_present(self) -> bool:
        return bool(self.settings.mt5_login and self.settings.mt5_password and self.settings.mt5_server)

    def is_available(self) -> bool:
        return self.settings.mt5_enabled and self._mt5_module is not None and self._credentials_present()

    def connect(self) -> dict[str, Any]:
        if not self.settings.mt5_enabled:
            event_bus.emit('mt5.mock.enabled', 'MT5Adapter', 'MT5 disabled; running mock mode', {'mt5_enabled': False})
            return {'ok': True, 'connected': False, 'mode': 'mock', 'reason': 'MT5_ENABLED=false'}

        if self._mt5_module is None:
            event_bus.emit('mt5.connection.failed', 'MT5Adapter', 'MetaTrader5 package unavailable; falling back to mock', {'package_available': False})
            return {'ok': False, 'connected': False, 'mode': 'mock', 'reason': 'MetaTrader5 package unavailable'}

        if not self._credentials_present():
            event_bus.emit('mt5.connection.failed', 'MT5Adapter', 'MT5 credentials missing; falling back to mock', {'credentials_present': False})
            return {'ok': False, 'connected': False, 'mode': 'mock', 'reason': 'MT5 credentials missing'}

        # Phase 02 shell: explicit no live connectivity handoff.
        return {'ok': True, 'connected': False, 'mode': 'adapter-shell', 'reason': 'Phase 02 uses mock-safe adapter shell'}
