...
def upgrade() -> None:
    ...
    if _table_exists("refresh_tokens"):
        with op.batch_alter_table("refresh_tokens", schema=None) as batch_op:
            batch_op.create_unique_constraint(
                batch_op.f("uq_refresh_tokens_token_hash"), ["token_hash"]
            )
            batch_op.create_foreign_key(
                batch_op.f("fk_refresh_tokens_user_id_users"),
                "users",
                ["user_id"],
                ["id"],
            )
    ...
    if _table_exists("scheduler_jobs"):
        ...
        _add_column_once(
            "scheduler_jobs",
            sa.Column(
                "schedule_type", sa.String(), nullable=False, server_default="interval"
            ),
        )
        _add_column_once(
            "scheduler_jobs",
        ...
        if not _index_exists("scheduler_jobs", "ix_scheduler_jobs_name"):
            op.create_index(
                "ix_scheduler_jobs_name", "scheduler_jobs", ["name"], unique=False
            )
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("scheduler_jobs", schema=None) as batch_op:
        ...
        batch_op.alter_column(
            "interval_seconds", existing_type=sa.INTEGER(), nullable=False
        )
    ...
    if _table_exists("refresh_tokens"):
        with op.batch_alter_table("refresh_tokens", schema=None) as batch_op:
            batch_op.drop_constraint(
                batch_op.f("fk_refresh_tokens_user_id_users"), type_="foreignkey"
            )
            batch_op.drop_constraint(
                batch_op.f("uq_refresh_tokens_token_hash"), type_="unique"
            )
