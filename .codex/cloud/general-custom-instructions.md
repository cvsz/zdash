...
5. Do not remove existing behavior unless the requested phase explicitly requires a migration.
...
9. Update README, docs, tests, scripts, and `.env.example` when adding features.
...
13. All new data must be tenant-scoped when tenancy, organizations, or workspaces exist.
...
16. Do not delete tests or weaken safety checks just to make CI pass.
17. If CI fails, inspect the exact workflow/job logs first and fix the root cause.
18. Use mocks, adapters, and dependency injection for unavailable external providers.
...
- Use `npm install --legacy-peer-deps --no-audit --fund=false` for frontend setup unless the lockfile and dependency graph are intentionally repaired.
...
If frontend dependency resolution fails, run:
...
When code is changed, include a concise commit-ready summary. Do not paste unnecessary giant file dumps if files are already written to the repo.
...
- Governance, certification, auditor portal, deployment packs, managed updates, fleet relay, AI Ops, data plane, marketplace, endgame release, sovereign cloud, and security ops must remain dry-run, read-only, or approval-gated by default.
...
- Security audit jobs may be non-blocking only when the workflow explicitly marks them non-blocking.
